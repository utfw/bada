...
모바일 웹 기반 3D 해양 체험. 고래상어와 물고기 떼가 유영하는 수중 씬을 감상합니다. 카메라가 고래상어를 부드럽게 자동 추적하며, 고래상어가 시야를 벗어나면 터치·마우스 드래그로 주변을 둘러볼 수 있습니다.
...
- **카메라** — 고래상어를 부드럽게 자동 추적(soft-follow). 고래상어가 시야를 벗어나면 모바일 터치 / 데스크탑 마우스 드래그로 주변을 둘러봄 (Pointer Events)
