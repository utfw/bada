...
- **[코드 검증] 문서 전용 목표도 완료 가능**: README/루트 CHANGELOG만 바꾸는 목표는 `src/` 변경이 0이라는 이유로 abandoned되면 안 된다 — 시각 Reviewer(탑뷰 등)는 스킵하되 `tsc --noEmit` 게이트(`runDocGate`)를 통과하면 완료로 인정하고, autoCommit이 문서를 stage해 실제 커밋한다. agent/** 인프라 파일은 여전히 stage 대상 아님(가드레일). <!-- @src: agent/loop.ts:runDocGate -->
