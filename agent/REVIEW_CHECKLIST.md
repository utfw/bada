...
- **✅ Fish forwardDot 부호 규약 — 음수가 정상 (2026-08-27 확정)**: `fish.avgForwardDot`이 **-1.00이면 정상**이다. 이것을 이상으로 보고하지 말 것.
  - **근거(코드로 확인)**: 물고기 메시는 머리가 로컬 **-Z**를 향하도록 조립되고(`Fish.ts` `createFishMesh`: `inner.rotation.y = π/2`로 +X 머리를 outer의 -Z에 정렬), `lookTarget`은 `pos - velocity`다. Three.js `getWorldDirection`은 규약상 **+Z** 축을 반환하므로(§1의 다른 항목도 "로컬 +Z"로 명시), 머리부터 정상 전진하는 물고기의 dot은 **-1**로 나온다.
  - **판정**: `avgForwardDot ≈ -1` → **정방향, 통과**. `avgForwardDot ≈ +1`(양수) → 꼬리부터 전진하는 **진짜 이상**이므로 그때만 사람에게 보고한다.
  - **이력**: 2026-04-19~2026-08-26 동안 Observer 라벨의 부호가 뒤집혀 있어 정상 상태를 매 실행 `⚠역방향(-1.00)`으로 오보했다. 그 오보 때문에 에이전트가 멀쩡한 `lookAt` 부호를 고치려다 retry 예산을 태웠고(§1 ⛔ 금지 규칙과 본 항목의 이전 HUMAN_VERIFICATION_REQUIRED 규정이 모두 그 오보의 산물), 라벨은 `agent/pipeline/observation.ts`에서 수정됨. **`lookAt` 수식 자체는 여전히 ⛔ 수정 금지** — 방향은 처음부터 옳았다.
...
<!-- @src: src/scene/GodRayPass.ts:GodRayPass -->
- **[코드 검증] 갓레이는 후처리(GodRayPass) 방식 — 씬 불변식**: 시각 광선은 `SceneManager`의 후처리 체인(`RenderPass → GodRayPass → OutputPass`)이 담당한다(스크린스페이스 light scattering + 각도 밴딩, 2026-07 확정). 검증 두 가지: (1) `SceneManager.ts`에 `composer.addPass(this.godRayPass)` 배선과 `GODRAY_EXPOSURE > 0`, (2) `GodRayPass.ts`의 `uExposure` 기본값 > 0 — 자동 수치 검증("GodRayPass 배선")이 이를 커버하므로 결과를 그대로 인용. **⛔ Ocean/Lighting에 지오메트리 기반 god ray(cone/plane/billboard 메시)를 추가하면 실패** — 과거 Ocean 빌보드 + Lighting cone/near-ray plane이 중복 존재해 후처리에 증폭되어 사각형 아티팩트를 냈고 제거됨. `Ocean.addGodRays()`는 실제 조명(SpotLight 4개)만 둔다. 밝기·갈래 조정은 `GodRayPass` uniform(uExposure/uThreshold/uBandCount/uBandStrength)과 SceneManager `GODRAY_EXPOSURE`로만.
...
- **[코드 검증] 변경 0 no-op 완료는 커밋 큐잉 금지**: goal이 완료됐는데 `src/`·문서 실변경이 0이면 커밋 대기열에 등록하지 않는다(안 한 일을 주장하는 가짜 COMMIT_MSG 방지). Reviewer는 변경 0 완료가 정당한 no-op인지 impl 사유에 명시됐는지 확인. <!-- @src: agent/loop.ts:executeGoal -->
- **[코드 검증] 완료 게이트는 내용 델타(blob hash)로 판정**: "이 goal이 실변경했나"는 파일 경로 집합 차분(`filesBefore.has(f)`)이 아니라 goal 시작 시점 대비 **blob hash 변화**로 판정해야 한다. 경로 집합만 보면 **선행 완료 goal이 dirty하게 남긴 파일을 후행 goal이 더 바꿔도 "변경 0"으로 오탐** → silent no-op abandoned로 폐기됨(2026-07-27 러너 로그에서 goal-02 makeFinMaterial이 이렇게 유실). <!-- @src: agent/pipeline/goals.ts:changedSinceSnapshot -->
- **[코드 검증] 문서 전용 목표도 완료 가능**: 루트 CHANGELOG만 바꾸는 목표는 `src/` 변경이 0이라는 이유로 abandoned되면 안 된다 — 시각 Reviewer(탑뷰 등)는 스킵하되 `tsc --noEmit` 게이트(`runDocGate`)를 통과하면 완료로 인정하고, autoCommit이 문서를 stage해 실제 커밋한다. agent/** 인프라 파일과 README는 stage 대상 아님(아래 항목 참조). <!-- @src: agent/loop.ts:runDocGate -->
- **[코드 검증] README·CLAUDE.md는 사람 소유 — 에이전트 수정 금지**: 프로젝트가 무엇인지 사람에게 설명하는 문서이므로 에이전트가 고쳐선 안 된다. 실제 사고: "사용자 지시사항 도큐먼트 최적화: README와 문서를 더 명확하고 간결하게 작성…" 목표가 생성·수행돼 README 113줄이 재작성·커밋됐다(`fb901a8`, 2026-08-16). 방어는 4겹 — ① `HUMAN_OWNED_DOC_FILES`가 `COMPLETION_DOC_FILES`와 분리돼 README-only 변경은 완료 산출물로 인정되지 않고 autoCommit이 stage하지 않는다 ② `FORBIDDEN_GOAL_PATTERNS`가 `readme`·"문서 최적화/간결화" 류 목표를 `appendGoals`에서 차단 ③ `harness-guard.sh`가 Edit/Write/Bash 리다이렉트를 `exit 2`로 사전 차단(절대경로·`./` 접두어 정규화 포함) ④ 완료 게이트가 `HUMAN_OWNED_DOC_FILES` 변경을 사후 감지해 `revertHarnessFiles`로 되돌리고 goal을 거부. 루트 `CHANGELOG.md`는 이력 문서라 계속 허용된다. <!-- @src: agent/pipeline/types.ts:HUMAN_OWNED_DOC_FILES -->
...
- **[코드 검증] 목표 생성기 자기수정 금지**: 에이전트 파이프라인(`agent/**`: loop/pipeline/vision/evolve/report 등)을 대상으로 하는 goal은 생성 금지 — "진화"의 대상은 오직 `src/` 제품 코드(씬·시각·Fish·WhaleShark·조명)다. 생성기가 `getRecentHumanCommits`를 관심 신호로 쓰는데 최근 사람 커밋이 `fix(agent)` 위주면 "Split loop.ts", "cost CV reporting", "exponential backoff" 같은 자기개선 메타 목표를 만들고, Implementer가 수행해도 autoCommit이 `agent/**`를 stage 안 해 워킹트리에 고이는 헛도는 루프가 된다(2026-08-12 러너 로그). `FORBIDDEN_GOAL_PATTERNS`가 `appendGoals` chokepoint에서 모든 생성경로를 차단하되, `agent/REVIEW_CHECKLIST.md` 갱신(Reviewer 정상 동작)과 `src/` 제품 목표는 통과시킨다. **단 시각 품질 '평가 기준' 개선(예: "god ray 품질을 존재 넘어 부피감까지 평가")은 진화의 정당한 방향이므로 차단하지 않는다 — `agent/vision` judge/label '코드·데이터'를 리팩터하는 목표만 막는다.** 또한 rate-limit/시각 문구가 오염된 헛것 목표("Remove unnecessary resets at 9:40am …")도 차단한다. <!-- @src: agent/pipeline/types.ts:FORBIDDEN_GOAL_PATTERNS -->
- **[코드 검증] 자원 한도 도달은 목표만 skip, 루프 중단 아님**: Planner/Implementer/Reviewer가 예산 캡(`error_max_budget_usd`)이나 턴 한도(`error_max_turns`)로 CLI가 중도 종료하면 그 목표만 `failed`로 두고 다음 목표로 진행한다 — code 실패가 아닌 자원 한도이므로 `interrupted`(exit 1, run-loop 중단)로 처리하면 안 된다. max_turns는 대개 목표가 실행 불가/모호해 Planner가 헤맨 경우다(2026-08-13 러너 로그: 헛것 목표 하나가 max_turns로 루프 전체를 죽임). <!-- @src: agent/loop.ts:stageFailureResult -->
- **[코드 검증] rate-limit 판정은 구조신호로만, 정상 응답 본문 텍스트로 판정 금지**: 일 사용량 한도(rate-limit)는 CLI가 **비정상 종료(throw)하는 catch 경로**나 `is_error=true`일 때만 본문 메시지(`isRateLimitMessage`)로 확인한다. **정상 생성 응답(is_error=false)은 본문에 rate-limit 문구를 인용하더라도 한도가 아니다** — 예: Planner가 FORBIDDEN 정규식이나 "resets 9:40am"·"rate-limit"을 설명하는 `PLAN_ABANDON`을 정상 출력하면(out>0, turns>0), 그 본문 매칭으로 rateLimited=true를 내면 안 된다. 본문 텍스트 매칭은 에이전트가 한도 문구를 설명할 때 재귀적 오탐을 낳아 가짜 code 75(30분 대기)로 이어진다(2026-08-13 러너 로그, metrics.jsonl 유일 rateLimited=true가 out=752 정상 생성이었음). <!-- @src: agent/pipeline/runner.ts:runClaudeOnce -->
- **[코드 검증] 하네스는 CLI 권한으로 물리적 잠금 (주 방어)**: 에이전트가 실행 하네스(`agent/**` 중 정책 파일 아닌 모든 것 — loop/pipeline/runner/vision judge/checks/evolve/observe/report 등)를 **수정하지 못하게 CLI PreToolUse hook(`agent/harness-guard.sh`)으로 사전 차단**한다. `runClaudeOnce`가 모든 단계 CLI 호출에 `--settings`로 이 가드를 배선하고, 가드가 Edit/Write/Bash의 대상이 하네스면 `exit 2`로 막는다(정책 파일·`src/`는 통과). 목표 문구가 아니라 **실제 파일 경로**를 보므로 표현 우회("loop.ts"→"루프.ts")가 불가 — `FORBIDDEN_GOAL_PATTERNS`(설득 계층 블랙리스트)가 계속 뚫리던 것을 대체하는 물리적 강제다. 정책 경계는 `types.ts:isHarnessFile`이 단일 출처(정책 = `AGENT_POLICY_FILES`: REVIEW_CHECKLIST.md, vision/labels.json). 가드가 뚫려도 완료 게이트가 `implChanged.filter(isHarnessFile)`로 사후 감지해 `revertHarnessFiles`로 되돌린다(defense-in-depth). <!-- @src: agent/pipeline/types.ts:isHarnessFile -->
- **[코드 검증] 커밋 성공 시점에 대기열 비우기 + push 재시도**: `autoCommitAndPush`는 `git commit` 성공 **직후** 대기열을 비운다. push 뒤에서 비우면, push가 거부될 때 대기열이 남아 goal마다 같은 backlog를 재커밋하고 bullet이 누적돼 **안 한 일을 주장하는 커밋 메시지**가 된다(2026-08-26: 8개 goal bullet을 단 커밋의 실제 `Changed:`는 `constants.ts` 하나). push는 `pull --rebase` 후 재시도하고, 끝내 실패하면 `agent/push-failed` 신호로 exit 1 — 로컬에만 커밋된 상태를 green run으로 위장하지 않는다. run 시작 시 `syncWithRemote()`가 원격을 먼저 반영해 거부 자체를 예방한다. <!-- @src: agent/pipeline/goals.ts:pushCommitted -->
- **[코드 검증] 신규 목표 중복 판정은 배치 내부까지, 판정은 claude로**: `deduplicateGoalsWithClaude`는 신규 후보를 기존 목표뿐 아니라 **서로 간에도** 비교한다. 배치 내 중복을 놓치면 "현재값의 1.5~2배" 같은 **상대 배수 목표가 곱해져** 상수가 의도 범위를 크게 벗어난다(2026-08-26: 같은 갓레이 노출 상향이 2건 등록돼 `GODRAY_EXPOSURE` 1.2→2.2→4.0, 3.3배). LLM은 중복 **그룹 분류만** 하고(`G:1,2` / `E:3`) 무엇을 남길지는 코드가 결정한다 — "하나만 남기고 지워라"를 모델에 맡기면 그룹을 통째로 지워 멀쩡한 목표까지 잃는다(실측). 이 판정은 로컬 7B가 실패하므로 `runClaudeText`를 직접 쓴다. <!-- @src: agent/pipeline/goals.ts:deduplicateGoalsWithClaude -->
- **[Reviewer 행동 규칙] 자동 수치 검증을 2회 이상 "오탐"으로 뒤집었다면 검증 코드가 버그다**: Reviewer가 같은 항목을 매 실행 수동으로 통과 처리하고 있다면, 그것은 그 항목을 예외 처리할 근거가 아니라 **`agent/checks/numeric.ts`를 고칠 신호**다. 실제로 2건이 이 방식으로 수개월간 방치됐다 — ① `GodRayPass 배선`은 수치가 `constants.ts`로 옮겨졌는데 정규식이 `SceneManager.ts`에서 숫자 리터럴을 찾고 있었고 ② `pectoral.rotation.x`는 `indexOf("createPectoralFins")`가 메서드가 아닌 **필드 주석**에 걸려 엉뚱한 영역을 스캔했다. 둘 다 항상 실패하는 죽은 검사였고 Reviewer 토큰만 태웠다. 심볼로 참조되는 값은 **정의처 파일에서** 읽고, 함수 영역은 **선언 정규식**으로 앵커링할 것. <!-- @src: agent/checks/numeric.ts:runNumericChecks -->
- **[코드 검증] 진화 신호에서 agent-scope 사람 커밋 제외**: `getRecentHumanCommits`(목표 생성기 입력)는 `<type>(agent):` 스코프 커밋과 ` [agent]$`·`agent auto-commit`을 관심 신호에서 제외한다. 파이프라인 인프라 사람 커밋을 "관심 영역"으로 주입하면 생성기가 self-mod 목표를 만드는 근원이 되므로, 진화 신호는 제품(`src/`) 커밋만 반영한다. <!-- @src: agent/pipeline/goals.ts:getRecentHumanCommits -->
...
- (2026-08-13) [human] §11 자기수정 금지 항목 추가 — 목표 생성기가 `getRecentHumanCommits`를 관심 신호로 쓰는데 최근 사람 커밋이 `fix(agent)` 위주라, "Split loop.ts into pipeline modules"·"cost CV reporting"·"exponential backoff" 같은 agent/** 자기개선 메타 목표를 스스로 생성했고 Implementer가 수행했으나 autoCommit 가드레일에 막혀 워킹트리에 고임(2026-08-12 러너 로그, 리팩터 폐기됨). `FORBIDDEN_GOAL_PATTERNS`(types.ts)에 agent/** 경로·파이프라인 파일명·인프라 개념어(vision judge/label, cost CV, backoff, planner/reviewer 개선, goal 생성기 등) 패턴 추가 → `appendGoals` chokepoint에서 전 생성경로 차단. `GOAL_GENERATION_EXCLUSIONS` 프롬프트에도 동일 지시. src/ 제품 목표·REVIEW_CHECKLIST 갱신은 통과(16케이스 테스트 확인). 진화 대상은 src/ 뿐.
- (2026-08-13) [human] §11 두 항목 보강 — ① 자원 한도 항목 신설: max_turns(`error_max_turns`)/budget 도달 시 그 목표만 skip이고 루프 중단(exit 1) 아님(2026-08-13 러너 로그: 헛것 목표가 Planner 15턴 소진 → stageFailureResult가 interrupted로 오분류해 루프 사망). Planner 턴 15→20 상향. ② FORBIDDEN 패턴 보강: 헛것(rate-limit 문구 오염 "resets Nam"), backfill CHANGELOG, report/checklist/stale-binding 등 놓친 self-mod 차단. **단 사용자 결정으로 시각 품질 '평가 기준' 개선("god ray 품질 평가")은 진화 방향이라 통과시키고, agent/vision judge/label '코드·데이터' 리팩터만 차단하도록 경계 설정(god-ray-quality 패턴은 넣었다가 제거).**
- (2026-08-13) [human] §11 rate-limit 판정 항목 추가 — 앞 커밋(FORBIDDEN "resets Nam" 패턴)의 부작용: Planner가 그 헛것 목표를 `PLAN_ABANDON`하며 정규식·"resets 9:40am"·"rate-limit"을 **정상 출력에 인용**하자, `runClaudeOnce`가 정상 응답 본문에 `isRateLimitMessage`를 적용해 가짜 rate-limit(code 75, 30분 대기)로 오판(러너 2026-08-13_11-13-35). 수정: 정상 응답(is_error=false)은 본문 무관하게 rateLimited=false, 실제 한도는 catch(throw)·is_error=true 경로에서만 감지. 본문 텍스트 매칭이 재귀적 오탐을 낳던 근본 결함 제거.
- (2026-08-15) [human] §11 하네스 물리 잠금 항목 2개 추가 — 블랙리스트(FORBIDDEN_GOAL_PATTERNS)가 표현 우회("loop.ts"→"루프.ts", "예산 초과 자원 확보")로 계속 뚫려 Implementer가 `agent/loop.ts`·`vision/*`·`checks/numeric.ts`를 반복 수정(러너 2026-08-13_15-11-00 goal 3). 방어를 "설득"에서 "물리적 강제"로 이동: CLI PreToolUse hook(`agent/harness-guard.sh`)을 `runClaudeOnce`가 `--settings`로 모든 단계에 배선, 하네스 경로 Edit/Write/Bash를 exit 2로 사전 차단(정책·src만 허용). 경계는 `isHarnessFile`(정책=AGENT_POLICY_FILES) 단일 출처. 완료 게이트에 `revertHarnessFiles` 사후 안전망. `getRecentHumanCommits`가 agent-scope 커밋을 진화 신호에서 제외. 라이브 검증: agent/loop.ts 편집 차단·src 편집 통과 확인.
