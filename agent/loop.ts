...
**실제로 새로 발견된 문제가 있을 때만** 아래 SUGGESTIONS 블록을 출력하세요. 최대 3개까지 권장합니다.
...
- 이미 goals.md의 완료 항목(\`[x]\`)이나 미완료 항목(\`[ ]\`/\`[~]\`)과 같은 파일·함수·동작을 다루는 제안은 금지
...
`.trim();
  ...
  return runClaude(prompt, "Read,Glob,Grep,Bash,Edit,Write", 20, "claude-opus-4-6");
}
...
function runGoal(goal: Goal, goalIndex: number, log: AgentLog, maxCycles: number): GoalResult {
  console.log(`\n${"═".repeat(60)}`);
...
function runStandaloneReview(maxCycles: number): void {
...
function runGoals(log: AgentLog, maxCycles: number): void {
  const goals = parsePendingGoals();
  if (goals.length === 0) {
  ...
  console.log(`미완료 목표: ${goals.length}개 (목표당 최대 체크리스트 사이클: ${maxCycles})\n`);

  let completed = 0;
  let stoppedByRateLimit = false;

  for (let i = 0; i < goals.length; i++) {
    const result = runGoal(goals[i], i, log, maxCycles);
    if (result === "completed") {
      completed++;
    }
...
function parseMaxCycles(args: string[]): number {
  const idx = args.findIndex((a) => a === "--max-cycles" || a === "-n");
  if (idx === -1) return MAX_CHECKLIST_CYCLES;
  const val = args[idx + 1];
  const n = Number.parseInt(val ?? "", 10);
  if (!Number.isInteger(n) || n <= 0) {
    console.error(`✗ --max-cycles에는 양의 정수가 필요합니다 (받은 값: "${val ?? ""}")`);
    process.exit(1);
  }
  return n;
}
...
function main() {
  ...
  if (maxCycles !== MAX_CHECKLIST_CYCLES) {
    console.log(`⚙  체크리스트 사이클 상한: ${maxCycles} (기본값 ${MAX_CHECKLIST_CYCLES} 오버라이드)`);
  }

  if (reviewOnly) {
    runStandaloneReview(maxCycles);
    return;
  }
  ...
  // 시작 전 미완료 목록 자체의 중복 정리
  deduplicateExistingGoals();

  const goals = parsePendingGoals();
  if (goals.length === 0) {
    console.log("중복 정리 후 남은 목표 없음.");
    return;
  }
