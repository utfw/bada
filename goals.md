...
- [x] `src/scene/GodRayPass.ts`의 uniform 조정으로 후처리 갓레이 가시성·형태 개선 — 광선이 흐리면 uExposure(또는 SceneManager `GODRAY_EXPOSURE`) 상향·uThreshold 하향, 갈래가 안 보이면 uBandStrength/uBandSharp 상향, 상단 광원이 어두우면 Ocean 배경 quad top color 밝기 보강. ⛔ Ocean/Lighting에 지오메트리 god ray 메시 추가 금지(씬 불변식)
- [ ] Remove unnecessary resets at 9:40am (Asia/Seoul) to improve performance.
...
- [ ] Align vision judge rubric to single-axis coverage for more accurate assessment of bubble surface cases.
- [ ] Add per-stage cost CV reporting and refine vision labels to improve transparency and data quality.
- [ ] Implement a vision judge with recall/precision against labeled Type C cases to enhance system accuracy.
- [ ] Simplify report padding and cache file reads in the checklist check to optimize performance.
- [ ] Add a deterministic stale-binding check for review checklist items to ensure consistency.
- [ ] Retry transient API overload using exponential backoff to improve reliability of system operations.
