...
- [ ] `Fish.ts:constructor()` schoolDefs: school1(`cz=8`)과 school4(`cz=14`)의 z 중심이 양수여서 기본 카메라 전방(-Z 방향)에서 이 두 학교가 화면 뒤에 위치해 보이지 않음. `school1 cz=8→-8`, `school4 cz=14→-8`로 변경하여 5개 school 모두 전방 반구(-Z)에 위치시키고 screenshot-3·4에서 물고기 군집이 우하단에 편중되어 보이는 문제 완화
