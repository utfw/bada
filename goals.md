...
- [ ] `Lighting.ts`에서 fill light(`new THREE.PointLight(0x5588bb, ...)`)의 intensity를 0.5→1.2로, position을 `(0,-15,0)`→`(0,-8,0)`으로 상향해 whaleshark-below.png에서 복부가 여전히 매우 어두운 실루엣으로 보이는 문제 개선 — 여러 사이클에 걸쳐 emissive와 fill light를 추가했지만 하면 앙각에서 복부가 아직 거의 검은색에 가까움
