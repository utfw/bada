  ...
  constructor() {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uDensity: { value: 1.0 },    // 광원 쪽으로 얼마나 멀리 샘플하나 (0~1)
        uWeight: { value: 1.0 },     // 샘플당 가중치
        uDecay: { value: 0.97 },     // 샘플당 감쇠 (스트리크 길이)
        uExposure: { value: 0.75 },  // 전체 세기 (SceneManager가 매 프레임 갱신)
        uThreshold: { value: 0.07 },  // 이 밝기 이상만 광선에 기여 (물고기 등 어두운 것 배제)
        uColor: { value: new THREE.Color(1.0, 1.0, 1.0) }, // 순백 틴트 (상단 광원 밝기 증폭)
        uTime: { value: 0 },         // 밴드 천천히 흐르게
        uBandCount: { value: 10.0 }, // 광원 기준 각도 밴드 개수 (갈래 수, 적을수록 넓은 광선)
        uBandSharp: { value: 8.0 },  // 밴드 대비 (클수록 또렷, 낮을수록 부드러움)
        uBandStrength: { value: 7.0 }, // 밴딩 강도 (0=균일 글로우, 1=완전 갈래)
      },
      vertexShader: /* glsl */`
        varying vec2 vUv;
        void main() {
      ...
      fragmentShader: /* glsl */`
        ...
        void main() {
          ...
          float falloff = max(0.0, 1.0 - pow(dist / 1.2, 0.5));
          // 수직 감쇠: 광원 근처(상단)에서 강하고 하단으로 갈수록 옅어져 기둥 부피감
          rays *= falloff * pow(clamp(vUv.y, 0.0, 1.0), 0.3);
          ...
          // 광원에서 멀어질수록 옅어짐(농담).
          float distFade = 1.0 - smoothstep(0.0, 0.9, length(dir));
