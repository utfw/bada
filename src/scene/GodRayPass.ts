  ...
  constructor() {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uExposure: { value: 6.0 },   // 전체 세기 (SceneManager가 매 프레임 갱신)
        uThreshold: { value: 0.15 },  // 이 밝기 이상만 광선에 기여 (물고기 등 어두운 것 배제)
        uColor: { value: new THREE.Color(0.9, 0.97, 1.0) }, // 연청색 틴트
        uTime: { value: 0 },         // 밴드 천천히 흐르게
        uBandCount: { value: 8.0 },  // 광원 기준 각도 밴드 개수 (갈래 수, 적을수록 넓은 광선)
        uBandSharp: { value: 8.0 },  // 밴드 대비 (클수록 또렷, 낮을수록 부드러움)
        uBandStrength: { value: 2.8 }, // 밴딩 강도 (0=균일 글로우, 1=완전 갈래)
      },
      vertexShader: /* glsl */`
        varying vec2 vUv;
        void main() {
