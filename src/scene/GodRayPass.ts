  ...
  constructor() {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uDecay: { value: 0.92 },     // 샘플당 감쇠 (스트리크 길이)
        uExposure: { value: 3.8 },    // 전체 세기 (SceneManager가 매 프레임 갱신)
        uThreshold: { value: 0.10 },  // 이 밝기 이상만 광선에 기여 (물고기 등 어두운 것 배제)
        ...
        uBandSharp: { value: 3.0 },  // 밴드 대비 (클수록 또렷, 낮을수록 부드러움)
        uBandStrength: { value: 0.50 }, // 밴딩 강도 (0=균일 글로우, 1=완전 갈래)
      },
      vertexShader: /* glsl */`
        varying vec2 vUv;
        void main() {
      ...
      fragmentShader: /* glsl */`
        ...
        void main() {
          ...
          rays *= falloff * pow(clamp(1.0 - vUv.y, 0.0, 1.0), 1.5);
